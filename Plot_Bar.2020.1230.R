...
f_barplot_onevar= function(data.arg, var_target, 
  ...
  if (coord_flip.arg == TRUE){
    p.final = p1+
          geom_text(aes(label=Percentage), vjust=0.8, hjust = 1.2, color="white", fontface="bold", position =  position_dodge(0.9), size=5)+
          coord_flip()
  }else{
    p.final = p1+
         geom_text(aes(label=Percentage), vjust=1.2, hjust = 0.8, color="white", fontface="bold", position =  position_dodge(0.9), size=5)
  }
